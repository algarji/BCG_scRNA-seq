...
pbmc.merged <- ScaleData(pbmc.merged, vars.to.regress = c("S.Score", "G2M.Score", "HTO_maxID"), features = rownames(pbmc.merged), block.size = 100) 		## en proximos analisis valorar si hacer regresion de percent.mt (tambien ribosomales)
...
cluster5 <- filter(cluster5.markers, avg_log2FC > 0.58 & p_val_adj < 0.05)
cluster6.markers <- FindMarkers(gd, ident.1 = 6, min.pct = 0.25)
cluster6 <- filter(cluster6.markers, avg_log2FC > 0.58 & p_val_adj < 0.05)

new.cluster.ids <- c("gd_resting", "gd_dying", "gd_TRDV1-like", "gd_eff", "gd_cycling", "gd_dying", "gd_CM")
